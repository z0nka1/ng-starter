...
export class ReuseTabService implements OnDestroy {
    ...
    private _cacheNewTab: any = {};
    private _currentCacheTabUrl: string = null;
    private removeUrlBuffer: string;
    _newTabChange: BehaviorSubject<any> = new BehaviorSubject<any>(null);
    // region: public
    ...
    /** 订阅缓存变更通知 */
    get newTabChange(): Observable<any> {
        return this._newTabChange.asObservable(); // .pipe(filter(w => w !== null));
    }
    /** 自定义当前标题 */
    ...
    /* 获取当前激活的tab cache对象 */
    current(): ReuseTabCached {
        return this._cacheNewTab[this._currentCacheTabUrl];
    }
    /**
     * 根据URL移除标签
     *
    ...
    constructor(
        ...
        /**
         * 路由策略RouteReuseStrategy接口的store方法，在页面第一次打开的时候，DetachedRouteHandle为null，无法获取当前页面的组件实例
         * 此方法是一个hack
         */
        this.newTabChange.subscribe(item => {
            this._cacheNewTab[this._currentCacheTabUrl] = item;
        });
    }
    ...
    private runHook(method: string, url: string, comp: any) {
        if (comp.instance && typeof comp.instance[method] === 'function')
            comp.instance[method](comp.instance);
    }
    ...
    store(_snapshot: ActivatedRouteSnapshot, _handle: any) {
        ...
        if (idx === -1) {
            item._handle2 = _handle;
            this._cached.push(item);
        } else {
            Object.assign(this._cached[idx], item);
        }
        this.removeUrlBuffer = null;
        ...
        // console.log(_.cloneDeep(this._cached[idx]));
    ...
    ngOnDestroy(): void {
        ...
        this._newTabChange.unsubscribe();
    }
}
