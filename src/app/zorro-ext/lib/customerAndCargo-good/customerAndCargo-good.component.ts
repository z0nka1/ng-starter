...
export const CUSTOM_INPUT_CONTROL_VALUE_ACCESSOR: any = {
    ...
    useExisting: forwardRef(() => CustomerAndCargoGoodComponent),
    multi: true
};
...
@Component({
    selector: `customerAndCargo-good`,
    template: `
    <div class="customerAndCargo-good">
        <nz-select 
            class="customerAndCargo-good-nz-select"
            [nzServerSearch]="true"
            nzShowSearch
            [style.width]="_width" 
    ...
    styles: [`
        ...
        .customerAndCargo-good-nz-select:hover +.close-icon {
            opacity: 1;
        }
        .customerAndCargo-good-select {
            position: relative;
        }
        .echo-loading {
            position: absolute;
...
export class CustomerAndCargoGoodComponent implements ControlValueAccessor, OnInit {
    ...
    ngOnChanges(changes: any) {
        if (changes.goodId && this.goodInfo.goodId) {
            this.firstNum = 1;
            this.loading = true;
            this.queryData(this.goodInfo.goodId, [], true);
        }
        if (changes.customerId) {
            this.firstNum = 1;
            this.loading = true;
            this.canQuery = true;
            this.queryData(this.goodInfo.goodId, []);
        }
    }
    ...
    queryData(searchText?: string, options?: Array<GoodOpt>, firstIn: boolean = false) {
        ...
        if (this.goodInfo.goodId) {
            params = [{ "page": this.firstNum, "size": this.rowsNum }, { productId: this.goodInfo.goodId, productType: this.goodInfo.goodType }];
        } else {
            params = [{ "page": this.firstNum, "size": this.rowsNum }, { customerId: this.customerId || 'xxxxxxxxxxxx', name: goodName }];
        }
        this.api.getComboAndCustomerGood(params).subscribe(json => {
            ...
            this.firstNum += 1;
            if (firstIn || this.goodInfo.goodId) {
...
@NgModule({
    ...
    declarations: [
        CustomerAndCargoGoodComponent
    ],
    exports: [CustomerAndCargoGoodComponent]
})
export class CustomerAndCargoGoodtModule { }
